...
import { addTradeCardAction } from '../actions'
...
export default function CardDialog({ cardModel }: CardModelProps) {
  ...
  const { mutate, isLoading, data, error } = useZact(addTradeCardAction)
  ...
  return (
    <Dialog key={cardModel.id} open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add your card to trade</DialogTitle>
          <DialogDescription>
            You are adding your card to trade. Please, fill the serial number
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          <Button
            onClick={() => mutate({ serial, want, cardId: cardModel.id })}
          >
            {isLoading && (
