...
import { addBuyCardAction } from '../actions'
...
export default function CardDialog({ cardModel }: CardModelProps) {
  ...
  const { mutate, isLoading, data, error } = useZact(addBuyCardAction)
  ...
  return (
    <Dialog key={cardModel.id} open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add your card to buy</DialogTitle>
          <DialogDescription>
            You are adding your card to buy.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          <Button onClick={() => mutate({ want, cardId: cardModel.id })}>
            {isLoading && (
